 ...
 * - Step editing for all tasks
 * - Report generation with notes
 * - Undo/Redo system
 * - Theme/subject/side-topic hierarchy
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.AK_DATA && Object.keys(window.AK_DATA).length > 0) {
        ...
        window.addEventListener('akDataLoaded', initApp);
        // Timeout fallback
        setTimeout(() => {
...
function updateXPLevel() {
    ...
    if (newLevel > state.level) {
        ...
        if (avatarRenderer) avatarRenderer.triggerLevelUp();
    }
...
function undo() {
    ...
    if (action.type === 'toggleStep') {
    ...
    } else if (action.type === 'editStep') {
        const task = state.activeTasks.find(t => t.id === action.taskId);
        if (task && task.steps[action.stepIndex]) {
            task.steps[action.stepIndex].text = action.oldText;
        }
        ...
        const task = state.activeTasks.find(t => t.id === action.taskId);
        if (task && action.stepData) {
            task.steps.splice(action.stepIndex, 0, action.stepData);
        }
...
function redo() {
    ...
    if (action.type === 'toggleStep') {
        ...
        const task = state.activeTasks.find(t => t.id === action.taskId);
        if (task && task.steps[action.stepIndex]) {
            task.steps[action.stepIndex].text = action.newText;
        }
    } else if (action.type === 'addStep') {
        const task = state.activeTasks.find(t => t.id === action.taskId);
        if (task && action.stepData) {
            task.steps.push(action.stepData);
        }
    } else if (action.type === 'deleteStep') {
        const task = state.activeTasks.find(t => t.id === action.taskId);
        if (task && task.steps[action.stepIndex]) {
            task.steps.splice(action.stepIndex, 1);
        }
    } else if (action.type === 'addTask') {
...
function handleStepEditClick(e) {
    ...
    const textarea = document.getElementById('stepEditText');
    ...
    newSaveBtn.onclick = () => {
        ...
        if (newText) {
            ...
            step.text = newText;
            pushUndo({ type: 'editStep', taskId, stepIndex, oldText, newText });
            saveState();
            renderActiveMissions();
        }
    ...
    newDeleteBtn.onclick = () => {
        if (confirm('Delete this step?')) {
            ...
            pushUndo({ type: 'deleteStep', taskId, stepIndex, stepData });
            saveState();
            renderActiveMissions();
        }
        modal.style.display = 'none';
...
function renderTaskBank() {
    ...
    container.innerHTML = filtered.slice(0, 30).map(t => `
        <div class="mission-item">
            ...
            ${t.steps ? `<div class="mission-meta">📋 ${t.steps.length} steps</div>` : ''}
            <div class="mission-actions">
    ...
    for (const day of last7Days) {
        ...
        if (dayCompletions.length === 0) {
            ...
            dayCompletions.forEach(c => {
                ...
                if (c.notes) html += `<br><span class="note-preview">📝 ${escapeHtml(c.notes.substring(0, 100))}${c.notes.length > 100 ? '...' : ''}</span>`;
                html += `</li>`;
            });
        }
        html += `</ul></div>`;
    ...
    if (state.completedHistory.length === 0) {
        ...
        state.completedHistory.forEach((c, idx) => {
            ...
            if (c.steps && c.steps.length > 0) {
                ...
                report += `   ✓ Steps: ${completedSteps}/${c.steps.length} completed\n`;
            }
            report += `\n`;
        });
    }
...
function setupGenerator() {
    ...
    // Generate chain
    const generateChainBtn = document.getElementById('generateChainBtn');
    if (generateChainBtn) {
        generateChainBtn.addEventListener('click', () => {
            const goal = document.getElementById('genGoal')?.value;
            if (!goal) {
                alert('Enter a goal first');
                return;
    ...
    if (addBtn) {
        addBtn.addEventListener('click', () => {
            if (window.lastGeneratedTask) {
            ...
            } else if (window.lastGeneratedChain) {
                window.lastGeneratedChain.tasks.forEach(task => {
                ...
                document.getElementById('generatedPreview').innerHTML = '';
                addBtn.style.display = 'none';
                showReward(`Added chain: ${window.lastGeneratedChain.goal} (${window.lastGeneratedChain.taskCount} tasks)`, 0, 0);
                window.lastGeneratedChain = null;
            }
        });
    }
}
...
function displayGeneratedTask(task) {
    ...
    task.steps.forEach(s => stepsHtml += `<li>${escapeHtml(s.text)}</li>`);
    stepsHtml += '</ul>';
...
function displayGeneratedChain(chain) {
    const previewDiv = document.getElementById('generatedPreview');
    if (!previewDiv) return;
    
    let tasksHtml = '';
    chain.tasks.forEach((task, idx) => {
        tasksHtml += `
            <div class="mission-item" style="margin-top: 10px;">
                <div class="mission-header">
                    <strong>${idx + 1}. ${escapeHtml(task.title)}</strong>
                    <div class="mission-badge">
                        <span class="badge ${task.difficulty.toLowerCase()}">${task.difficulty}</span>
                    </div>
                </div>
    ...
    previewDiv.innerHTML = `
        <div class="mission-item">
            <div class="mission-header">
                <strong>🔗 Mission Chain: ${escapeHtml(chain.goal)}</strong>
                <div class="mission-badge">
                    <span class="badge">${chain.taskCount} tasks</span>
                </div>
            </div>
            <div class="mission-meta">
                <span>📅 Horizon: ${chain.horizon}</span>
                <span>📁 ${chain.theme}</span>
                <span>⭐ Total XP: ${chain.totalXp}</span>
                <span>⏱️ Total Time: ${chain.totalEstimatedTime} min</span>
            </div>
            ${tasksHtml}
        </div>
    `;
    
    window.lastGeneratedChain = chain;
    window.lastGeneratedTask = null;
    document.getElementById('addGeneratedBtn').style.display = 'inline-block';
}
...
function initApp() {
    ...
    if (typeof AkatsukiAvatarRenderer !== 'undefined') {
